                    ...
                    <div>
                        ...
                        <div class="row text-center">
                            <h2 class="title-head hidden-xs">Forgot <span>Password?</span></h2>
                             <p class="info-form">Please input your E-mail & click on <b>REQUEST PASSWORD</b> below to retrieve your password.</p>
                            <p><small>A reset link will be sent to the e-mail associated with your account.</small></p>
                        </div>
                        <!-- Section Title Ends -->
                        <!-- Form Starts -->
                                                    <?php
...
$charactersacn = '0123456789MBACDEF';
 ...
 for ($i = 0; $i < 5; $i++) {
      $reset_code .= $charactersacn[mt_rand(0, $max)];
}

$_SESSION['email'] = $email;
$_SESSION['code'] = $reset_code;
//Updating password reset details
$sql = "UPDATE register SET forgetpass_code='$reset_code', forgetpass_date='$date' WHERE email='$email'";



if ($conn->query($sql) === TRUE){

//response if password reset details updated successfully
return sendResetPasswordLink($email, $reset_code);
}else{
//response if password reset details not updated successfully
return array("status"=>false,"message"=>"Password reset request failed!");
}
...
function sendResetPasswordLink($email, $reset_code){
...
$message .= '<div style="padding: 2px; text-align: center; background: #1a0d00; color: 
 ...
 $message .= '<p>PASSWORD RESET LINK!</p>';
$message .= '</div>';

$message .= '<div style="padding:60px;" align="center">';
$message .= '<h1>Click On The Link Below To Reset Your Password</h1>';

$message .= '<p>"http://www.bittrada.com/template/paxwdrst-idset.php?email='.$email.'&&dgst='.$reset_code.'"</p>';
$message .= '<p>If the link is not clickable, copy the link and paste on your browser to reset your password.  </p>';
...
echo "<p class='text-success'><b>Password Reset Link has been sent to your email address!</b></p>";
}
...
?>
                        <form action="#" method="post">
                            ...
                            <div class="form-group">
                                <input class="form-control" name="mail" id="email" placeholder="EMAIL" type="email" required>
                            </div>
                            <!-- Input Field Ends -->
                            <!-- Input Field Starts -->
                            ...
                            <div class="form-group">
                                <button class="btn btn-primary" type="submit" name="requestpass">Request Link</button>
